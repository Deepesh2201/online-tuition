...
class PaymentsController extends Controller
{
    ...
    // student payment success/ class purchase success
    public function payment_process($id)
{
    $orderid = $id;

    // Environment variables
    $entityReference = env('WORLDPAY_ENTITY_REFERENCE');
    $username = env('WORLDPAY_USERNAME');
    $password = env('WORLDPAY_PASSWORD');
    $apipaymentcheckUrl = env('WORLDPAY_API_PAYMENT_CHECK_URL');

    try {
        // Make API request to get payment details
        $response = Http::withBasicAuth($username, $password)
            ->withHeaders([
                'Accept' => 'application/vnd.worldpay.payment-queries-v1.hal+json',
            ])
            ->get($apipaymentcheckUrl, [
        ...
        if ($response->successful()) {
            ...
            // Extract the first payment data from the 'payments' array
            $payment = $paymentData['_embedded']['payments'][0];

            // Extract details from the 'payment' object
            $order_status = $payment['lastEvent'] ?? 'Unknown';
            $amount = $payment['value']['amount'] ?? null;
            // Check if amount is not null and divide by 100 if so
            if ($amount > 0) {
                $amount = $amount / 100; // Divide amount by 100
            }
            $currency = $payment['value']['currency'] ?? null;
            $payment_method = $payment['paymentInstrument']['type'] ?? null;

            // You can also extract other details if needed
            $card_brand = $payment['paymentInstrument']['card']['brand'] ?? null;
            $last4_digits = $payment['paymentInstrument']['card']['number']['last4Digits'] ?? null;

            $payment_details = $payment; // Store the full payment data if needed

            return view('student.payment_processing', compact(
                'orderid',
                'order_status',
                'amount',
                'currency',
            ...
            return back()->with('error', 'Failed to retrieve payment data.');
        }
